#!/usr/bin/env bash
# Build and deploy Saral Job Viewer (API + UI + Redis).
#
...
#
# Docker lives at repo root (docker/ + docker-compose.yml).
# Secrets/config: backend/.env
#
# Requires SARAL_DOMAIN + SARAL_SSL_EMAIL in backend/.env.
# If host port 80 is already in use, deploy installs a host vhost + certbot --nginx.
set -euo pipefail
...
if [[ ! -f "${BACKEND_ROOT}/.env" ]]; then
  err "Missing backend/.env — copy from backend/.env.example and fill in values."
  exit 1
fi
if [[ ! -d "${REPO_ROOT}/frontend" ]]; then
  err "Missing frontend/ at ${REPO_ROOT}/frontend"
  exit 1
fi
...
GENERATED_DIR="$REPO_ROOT/docker/generated"
NGINX_OUT="$GENERATED_DIR/nginx.saral.conf"
NGINX_TEMPLATE="$REPO_ROOT/docker/nginx.saral.conf.template"
HOST_NGINX_TEMPLATE="$REPO_ROOT/docker/nginx.saral.host.conf.template"
...
if [[ -z "$SARAL_DOMAIN" ]]; then
  err "SARAL_DOMAIN is required in backend/.env (e.g. saral.thatinsaneguy.com)."
  exit 1
fi
...
ensure_host_ssl_cert() {
  ...
  if [[ -z "$SARAL_SSL_EMAIL" ]]; then
    err "SARAL_SSL_EMAIL is required in backend/.env to obtain a Let's Encrypt certificate."
    exit 1
  fi
...
ensure_docker_ssl_cert() {
  ...
  step "Obtaining Let's Encrypt certificate (standalone, temporary :80)…"
  info "DNS must resolve to this server and ports 80/443 must be reachable."
...
info "Compose: ${REPO_ROOT}/docker-compose.yml"
...
Validation image ready (not started): saral-dvalidate:latest
  ...
  docker run --rm --network saral-job-viewer_sjv-net --env-file ${BACKEND_ROOT}/.env saral-dvalidate:latest -1
EOF
