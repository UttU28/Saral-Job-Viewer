...
def qualificationTagsAsList(value: object) -> list[str]:
    if value is None:
        return []
...
def buildJobDescription(job: dict) -> str:
    ...
    if job.get("salaryRange"):
        prefix.append(f"Compensation (from listing): {job['salaryRange']}")
    if job.get("visaOrMatchNote"):
        prefix.append(f"Visa / match note: {job['visaOrMatchNote']}")

    body = (job.get("jobResponsibility") or "").strip()

    tags = qualificationTagsAsList(job.get("qualificationTags"))
    suffix = ""
    if tags:
        suffix = "## Skills\n" + "\n".join(f"- {tag}" for tag in tags)

    sections = [part for part in ("\n".join(prefix).strip(), body, suffix) if part]
    return "\n\n".join(sections).strip()
...
def certificationsRequired(tags: list[str], jobDescription: str) -> bool:
    blob = " ".join(tags).lower() + " " + jobDescription.lower()
    certMarkers = (
...
def buildCheckPayload(job: dict) -> dict:
    ...
    payload: dict[str, object] = {
        ...
        "cloud_specialization": inferCloudSpecialization(tags),
        ...
        if certificationsRequired(tags, jobDescription)
        else "",
    }
...
def loginAndCheck() -> None:
    ...
    if not email or not password:
        raise ValueError("Set MIDHTECH_EMAIL and MIDHTECH_PASSWORD in rest/.env")
