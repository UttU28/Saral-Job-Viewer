...
def resolveGlassdoorOutputPath():
    perSource = os.getenv("OUTPUT_JSON_GLASSDOOR")
    if not isinstance(perSource, str) or not perSource.strip():
        raise ValueError(
            "Set OUTPUT_JSON_GLASSDOOR in .env to the Glassdoor jobs JSON path "
            "(e.g. opGlassDoor.json)."
        )
...
def scrapeDetailPane(driver) -> tuple[str, str | None, str]:
    """Returns (job_description_text, website_url_or_none, company_from_header)."""
    ...
    try:
        a = driver.find_element(
            By.CSS_SELECTOR,
            '[data-test="job-details-header"] a[class*="EmployerProfile_profileContainer"], '
            'header a[class*="EmployerProfile_profileContainer"]',
        )
        href = a.get_attribute("href")
        website = absoluteUrl(driver, href)
    except NoSuchElementException:
        pass
    ...
    return description, website, company
...
def scrapeGlassdoorSearch(
    ...
    (from prior output JSON). Rows already saved are not clicked.
    """
    ...
    for idx in range(n):
        try:
            ...
            description, website, companyHdr = scrapeDetailPane(driver)
